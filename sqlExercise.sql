...
SELECT lh.MaLH, lh.TenLH, MAX(GiaBan) AS GiaBanCaoNhat
FROM MatHang mh
JOIN LoaiHang lh
ON mh.MaLH = lh.MaLH
JOIN ChiTietMatHang ctmh
...
-- 31. 
SELECT ctdh.MaDH, SUM(ctdh.SoLuong*ctmh.GiaBan) AS TongTien
...
SELECT * FROM chitietdonhang;
SELECT * FROM chitietMatHang;
SELECT ctdh.MaDH, GROUP_CONCAT(mh.TenMH,':', ctmh.GiaBan, ':',ctdh.SoLuong) as ChiTietDonHang, SUM(ctdh.SoLuong*ctmh.GiaBan) AS TongTien
FROM ChiTietDonHang ctdh
JOIN ChiTietMatHang ctmh
ON ctdh.MaMH = ctmh.MaMH
JOIN MatHang mh
