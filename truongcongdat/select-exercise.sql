...
SELECT TEMP.TenLH, TEMP.TenMH, MAX(TEMP.SoLuongMH)
FROM 
(
	SELECT LoaiHang.MaLH, TenMH, TenLH, SUM(ChiTietMatHang.SoLuong) AS SoLuongMH
	FROM LoaiHang
...
-- FUNCTION
DELIMITER $$
CREATE FUNCTION total(MaDonHang INT) RETURNS DOUBLE
BEGIN
DECLARE sum DOUBLE DEFAULT 0.0;
SELECT SUM(ChiTietMatHang.GiaBan * ChiTietDonHang.SoLuong) INTO sum
FROM DonHang
JOIN ChiTietDonHang ON ChiTietDonHang.MaDH = DonHang.MaDH
JOIN ChiTietMatHang ON ChiTietMatHang.MaMH = ChiTietDonHang.MaMH
WHERE DonHang.MaDH = MaDonHang
GROUP BY DonHang.MaDH;
RETURN sum;
END $$
