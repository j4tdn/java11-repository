...
public class App {
	...
	public static void main(String[] args) {
		// Câu 1:List all employees by department_id
		List<Employees> employees = employeeService.getEmloyeeById(deptId);
		print(employees);
		
	        // Câu 2:List all employees of each department and sorting dept_id asc
		List<Employees> Oremployees = employeeService.getAllEmp();
		print(Oremployees);
		
	        // Câu 3:List all project have budget > 400
		List<Project> projects = projectService.getAllProject(budget);
		print(projects);
		
		// Câu 4:List total budget of each department in 2020"
		List<DepartmentDto> departList = deptService.getDeptBudget(year);
		print(departList);
		
		// Câu 5:department which has project with highest budget in 2020"
