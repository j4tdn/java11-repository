...
CREATE TABLE KickCo(
    ...
    CONSTRAINT PK_KickCo PRIMARY KEY (MaKC)
);
...
CREATE TABLE ChiTietMatHang(
    ...
    CONSTRAINT FK_ChiTietMatHang_KickCo FOREIGN KEY (MaKC) REFERENCES KickCo(MaKC)
);
...
-- Phân biệt Truncate(not rollback, no filter) và Delete(rollback, filter(có điều kiện - Where))
