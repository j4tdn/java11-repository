...
public class App {
	public static void main(String[] args) {
		...
		// (thamso) -> {thanham // overrid}
		Book [] result = getBooks(books, (book -> book.getPublisher().contains("NhiDong")));
		show(books);
	...
	private static double getMoney(Book[] books) {
		...
		for (Book book: books) {
			...
			if (book.getId().contains("SGK") && ((TextBook)book).getStatus().equals(Status.OLD)) {
				realCost *= 0.7d;
