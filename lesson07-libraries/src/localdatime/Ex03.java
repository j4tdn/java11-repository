...
public class Ex03 {
	private static String LocalsizeKey = "dd.MM.yyyy'T'KK:mm:ss a";
	private static DateTimeFormatter dtf = DateTimeFormatter.ofPattern(LocalsizeKey);
	
	public static void main(String[] args) {
		LocalDateTime datetime = LocalDateTime.now();
		System.out.println("datetime: " + datetime);
		
		LocalDateTime anotherLDT =  LocalDateTime.of(LocalDate.now(), LocalTime.now());
		System.out.println("anotherLDT: " + anotherLDT);
		
		System.out.println("formated: " + dtf.format(datetime));
		
		//period used for LocalDate
		//duration used for LocalTime
		//          LocalDateTime
		
		LocalDateTime dtStart = LocalDateTime.of(2021, 10, 8, 10, 20, 10);
		LocalDateTime dtEnd   = LocalDateTime.of(2021, 10, 10, 20, 22, 30);
	...
	private static String between(LocalDateTime dtStart, LocalDateTime dtEnd) {
		...
		if(dtStart.isAfter(dtEnd)) {
			throw new IllegalArgumentException("StartDate can't exceed EndDate");
		}
