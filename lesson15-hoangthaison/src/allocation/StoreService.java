...
public class StoreService {
	 ...
	 */
	private static List<Store> list = getStores().stream().filter(e -> (e.getSelected() == true))
	...
	public static void main(String[] args) {
		...
		// Số store có expected sales != null
		Long count = list.stream().filter(e -> e.getExpectedSales() != null).count();

		for (Store store : list) {
			// Đoạn này nhìn code không được đẹp lắm :-)
			// store.getExpectedSales() == null tồn tại trong 3 if có thể gom lại
			// sumExpectedSale:: Giá trị sum này sẽ ko thay đổi trong quá trình em duyệt
			// Nên tạo 1 biết ngoài vòng for để lưu lại
			...
			if (store.getExpectedSales() == null && getExpectedSalebyID(store.getReferenceStoreId()) == null) {
				// sum của expected sales đang bị sai
				store.setExpectedSales(Lamtron(sumExpectedSale() / count, 1));
			}
			if (store.getExpectedSales() == null && store.getReferenceStoreId() != null) {
		...
		// Kết quả step2 đang bị sai
		mapAlocate.entrySet().forEach(System.out::println);

	}

	// Đừng đặt tên hàm bằng tiếng việt
	// >> rounding
	private static BigDecimal Lamtron(Double x, int index) {
	...
	private static Double sumExpectedSale() {
		...
		// BigDecimal total = list.stream().map(e-> e.getExpectedSales()).reduce(0,
		// BigDecimal::longValue);
		
		// Cách trên đúng rồi. Sao em ko dùng stream tính sum cho khỏe
		Double sum = 0d;
		for (Store store : list) {
	...
	private static BigDecimal getExpectedSalebyID(Long ID) {
		...
		// Optional ExpectedSale = list.stream().filter(e -> e.getStoreId()==
		// ID).map(Store::getExpectedSales).findFirst();
	...
	private static Map<Long, Integer> doAllocation(Integer whAllocationAmount, List<Store> data) {
		...
		for (Store store : list) {
			mapAlocation.put(store.getStoreId(),
					amountAlocation(new BigDecimal(whAllocationAmount), null, null).intValue());
		}
