...
public class StoreService {
    ...
    public static void main(String[] args) {
    	...
    	// Thực hiện tính toán
    	List<Store> missingExpectedSales = data.stream()
    			.filter(e -> e.getExpectedSales() == null)
    			.collect(Collectors.toList());
    	
    	
    	List<Store> rfStore = data.stream()
    			.filter(s -> s.getStoreId().equals(s.getReferenceStoreId()))
    			.collect(Collectors.toList());
    	...
    	missingExpectedSales.forEach(System.out::println);
    	System.out.println("====");
    	rfStore.forEach(System.out::println);
